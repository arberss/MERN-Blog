...
const LOG_LEVEL = LOG_LEVELS[(process.env.LOG_LEVEL || process.env.REACT_APP_LOG_LEVEL || "").toUpperCase()] || LOG_LEVELS.WARN;
...
class Logger {
	...
	_log(level, exceptionCode, message, args) {
		...
		if (isBrowser()) {
			...
			return args.length > 0 ? console[BROWSER_LOGGER[level] || level](message, ...args) : console[level](message);
		}
		// eslint-disable-next-line no-console
